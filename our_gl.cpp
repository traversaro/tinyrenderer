...
void triangleClipped(mat<4, 3, float> &clipc, mat<4, 3, float> &orgClipc, IShader &shader, 
	RenderBuffers& render_buffers, const Matrix &viewPortMatrix, int objectUniqueId)
{
		...
		for (P.y = bboxmin.y; P.y <= bboxmax.y; P.y++)
		{
			...
			double frag_depth = 0;
			{
				...
				if (bc_screen.x < 0 || bc_screen.y < 0 || bc_screen.z < 0 ||
					render_buffers.zbuffer[P.x + y_inverted * render_buffers.m_width] > frag_depth)
					continue;
			}
			...
			{
				render_buffers.zbuffer[P.x + y_inverted * render_buffers.m_width] = frag_depth;
				if (render_buffers.segmentation_mask.size())
				{
					render_buffers.segmentation_mask[P.x + y_inverted * render_buffers.m_width] = objectUniqueId;
				}
				render_buffers.rgb[3*(P.x+y_inverted*render_buffers.m_width)+0]=color[0];
				render_buffers.rgb[3*(P.x+y_inverted*render_buffers.m_width)+1]=color[1];
				render_buffers.rgb[3*(P.x+y_inverted*render_buffers.m_width)+2]=color[2];
			}
		}
	}
}
